...
def hybrid_vae_loss(recon, x, mu, logvar, kl_weight=1.0):
    ...
    l1_loss = nn.L1Loss()(
        recon, x
    )  # L1 loss: 이미지의 윤곽이나 구조적 특징 복원에 유리
    recon_loss = 0.5 * (mse_loss + l1_loss)  # mse loss와 L1 loss복합으로 사용함
        ...
        for i, (imgs, _) in enumerate(train_loader):
            ...
            if i % 100 == 0:
                print(
                    ...
                    f"VAE_loss: {vae_loss.item():.4f}, DDPM_loss: {ddpm_loss.item():.4f}, total: {total.item():.4f}"
                )
