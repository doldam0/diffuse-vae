...
def compute_metrics(
    ...
    with tqdm(
        range(num_batches), total=num_batches, desc="Generating for Metrics"
        ...
        for batch_idx in pbar:
            ...
            if batch_idx == 0:
                comparison_fake = samples[:16]
                comparison_real = real_images[:16]

                save_path = os.path.join(
                    os.path.dirname(comparison_save_path),
                    "visual_metrics_comparison.png",
                )
                comparison = torch.cat(
                    [comparison_real, comparison_fake], dim=0
                )
                comparison = comparison.to(torch.float32)
                vutils.save_image(
                    comparison,
                    save_path,
                    nrow=16,
...
def visualize_latent_space_pca(
    ...
    subset = Subset(dataset, subset_indices)  # type: ignore
